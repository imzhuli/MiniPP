...
void LoadForcedRelayerServers() {
	...
	for (auto & Line : Contents) {
		...
		auto ControlAddress = xNetAddress::Parse(TL[1]);
		...
		RuntimeAssert(ControlAddress && ControlAddress.Port, "ControlAddress should be valid address with valid port");
		RuntimeAssert(DataAddress && DataAddress.Port, "ControlAddress should be valid address with valid port");
		...
		Info.ControlAddress      = ControlAddress;
...
static auto                   InitTest = xInstantRun([] {
    RelayForTest.ControlAddress = xNetAddress::Parse("192.168.5.112:17000");
